...
train.to_csv('train_lon_lat_xgb_predicted.csv')
test.to_csv('test_lon_lat_xgb_predicted.csv')
...
checkpoint = ModelCheckpoint(filepath='lon_tmp_best.hdf5',
                             verbose=1,
                             save_best_only=True,
...
lon_model.load_weights('lon_tmp_best.hdf5')
lon_pred = lon_model.predict(lon_test.as_matrix()) 

earlystopping = EarlyStopping(monitor='loss', patience = 100., verbose=1, mode='min')
checkpoint = ModelCheckpoint(filepath='lat_tmp_best.hdf5',
                             verbose=1,
                             save_best_only=True,
                             save_weights_only=True,
                             monitor='loss',
...
lat_model.fit(lat_train.as_matrix(), lat_target.as_matrix(),batch_size=5000,
              epochs=epoch_nb,callbacks=[earlystopping,checkpoint])
lat_model.load_weights('lat_tmp_best.hdf5')
